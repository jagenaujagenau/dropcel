...
import { makeWatchStream } from "./watch-stream";

/**
 * Plumbing tests: a fake Tauri-shaped event queue drives the batch pump
 * without touching real Tauri. The reconciler's own decision logic is
 * covered in reconciler.test.ts — these only assert that every Tauri batch
 * reaches the handler whole, in arrival order, one at a time, and that scope
 * close actually tears the pump down. Batch atomicity matters: the rename
 * heuristic needs a vanished dir and an appeared dir in the SAME
 * `handleFsChanges` call, so splitting a batch across deliveries would let
 * two halves of one rename race two concurrent `reconcile()` scans.
 */
...
describe("watch-stream", () => {
  it.effect("delivers a batch to the handler whole, undivided", () =>
    Effect.gen(function* () {
      ...
      const watchStream = yield* makeWatchStream({
  ...
  it.effect("delivers successive batches in arrival order, one at a time", () =>
    Effect.gen(function* () {
      ...
      const watchStream = yield* makeWatchStream({
        ...
        onChanges: (changes) =>
          Effect.gen(function* () {
            const all = yield* Ref.updateAndGet(order, (xs) => [...xs, changes[0].project]);
